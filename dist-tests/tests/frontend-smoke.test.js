...
test('AdminPageHero renders title, description and stats', () => {
    const output = renderToStaticMarkup(_jsx(AdminPageHero, { icon: FolderKanban, title: "Painel", description: "Resumo do estado atual", stats: [
...
test('PublicMediaListItem renders the link and metadata', () => {
    const output = renderToStaticMarkup(_jsx(MemoryRouter, { children: _jsx(PublicMediaListItem, { title: "Sessao Aberta", href: "/laboratorio-cultural/sessoes/1", imageUrl: "/media/example.jpg", imageAlt: "Sessao aberta", dateTime: "2026-05-11", formattedDate: "11/05/2026", description: "Descricao da sessao" }) }));
...
test('PaginatedCollection renders the current slice of items', () => {
    const output = renderToStaticMarkup(_jsx(MemoryRouter, { initialEntries: ['/admin?page=2'], children: _jsx(PaginatedCollection, { items: ['A', 'B', 'C', 'D', 'E'], isLoading: false, error: "", loadingMessage: "A carregar", emptyMessage: "Vazio", itemsPerPage: 2, renderItem: (item) => _jsx("div", { children: item }, item) }) }));
...
test('LogsPage renders the logs dashboard shell', () => {
    const output = renderToStaticMarkup(_jsx(LogsPage, {}));
